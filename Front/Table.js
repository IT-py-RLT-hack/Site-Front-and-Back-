...
xhr.onload = function() {
    ...
    // Обработчик отправки формы для сортировки таблицы
    const sortForm = document.querySelector('form');
    sortForm.addEventListener('submit', (event) => {
        event.preventDefault();
        const columnSelect = sortForm.elements.column;
        const columnIndex = Object.values(columnIndexes).indexOf(columnSelect.value);
        const valueInput = sortForm.elements.value;
        const searchValue = valueInput.value.trim().toLowerCase();
        ...
        // Поиск и выделение строки, содержащей значение для сортировки
