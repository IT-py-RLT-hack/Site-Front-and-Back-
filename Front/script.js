  ...
  complete: function(results) {
    ...
    visibleCompanies.forEach(company => {
      ...
      detailsButton.addEventListener("click", () => { // Добавляем обработчик нажатия на кнопку
        ...
        companyName.innerText = selectedCompany.n;
      ...
      if (scrollTop + clientHeight >= scrollHeight && nextIndex < allCompanies.length) {
        ...
        newCompanies.forEach(company => {
          ...
          detailsButton.addEventListener("click", () => { // Добавляем обработчик нажатия на кнопку
            ...
            nameText.innerText = selectedCompany.n;
            ...
            const amountDueText = document.createElement("p");
            amountDueText.innerText = `Сумма задолженности: ${selectedCompany.amount_due}`;
            modalContent.appendChild(amountDueText);

            const contractPriceText = document.createElement("p");
            contractPriceText.innerText = `Стоимость контракта в рублях: ${selectedCompany.contract_price_rub}`;
            modalContent.appendChild(contractPriceText);

            const subcontractorPercentsText = document.createElement("p");
            subcontractorPercentsText.innerText = `Доля субподрядчиков в выполнении работ: ${selectedCompany.subcontractor_sum_percents}%`;
            modalContent.appendChild(subcontractorPercentsText);

            const executionEndDateText = document.createElement("p");
            executionEndDateText.innerText = `Дата окончания исполнения контракта: ${selectedCompany.execution_end_date}`;
            modalContent.appendChild(executionEndDateText);

            const eDocExecutionCountText = document.createElement("p");
            eDocExecutionCountText.innerText = `Количество исполненных контрактов через ЕСИА: ${selectedCompany.e_doc_execution_count}`;
            modalContent.appendChild(eDocExecutionCountText);
